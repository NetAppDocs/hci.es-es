...
. Configure su BMC, consulte link:hci_prereqs_final_prep.html["Configure IPMI para cada nodo"] para obtener más detalles.
. Consulte el artículo de la base de conocimientos de NetApp https://kb.netapp.com/onprem/solidfire/Element_OS/How_to_disable_BMC_web_UI_access_on_HCI_and_SolidFire_H-Series_systems["Cómo deshabilitar el acceso de interfaz de usuario web de BMC en sistemas HCI y SolidFire H-Series"^] Para determinar si es necesario desactivar el acceso web de BMC.
...
== Actualice el firmware del nodo

Después de sustituir el nodo de computación, es posible que tenga que actualizar la versión de firmware. Consulte link:task_hcc_upgrade_compute_node_firmware.html["Actualizar el firmware del nodo de computación"] para obtener más detalles.
