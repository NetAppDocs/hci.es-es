...
Si va a realizar una actualización in situ de un nodo testigo existente a Element 12.8 o posterior, antes de actualizar, debe asegurarse de que la versión del hardware de la máquina virtual en el nodo testigo sea compatible con ESXi 6.7 (versión de hardware de la máquina virtual 14) o posterior, según su entorno. Verlink:task_hcc_upgrade_management_node.html#upgrade-vm-hardware["Actualizar la versión de hardware de la máquina virtual"] .
